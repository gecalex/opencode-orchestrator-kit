...
exports.mcpRegistry = exports.MCP_CATEGORIES = exports.MCP_REGISTRY = void 0;
exports.findMCPServersByTechnology = findMCPServersByTechnology;
exports.findMCPServersByCategory = findMCPServersByCategory;
exports.findMCPServersByTag = findMCPServersByTag;
exports.getMCPServer = getMCPServer;
exports.getAllMCPServers = getAllMCPServers;
exports.getMCPServersGroupedByCategory = getMCPServersGroupedByCategory;
...
function findMCPServersByTechnology(tech) {
    const techMap = {
    ...
    if (categories.length === 0)
        return [];
    return exports.MCP_REGISTRY.filter(s => categories.includes(s.category));
}
// === Поиск MCP по кате��ор��и ===
function findMCPServersByCategory(category) {
    return exports.MCP_REGISTRY.filter(s => s.category === category);
}
// === Поиск по тегу ===
function findMCPServersByTag(tag) {
    return exports.MCP_REGISTRY.filter(s => s.tags.some(t => t.toLowerCase().includes(tag.toLowerCase())));
}
// === Получить MCP по ID ===
function getMCPServer(id) {
    return exports.MCP_REGISTRY.find(s => s.id === id);
}
...
function getMCPServersGroupedByCategory() {
    const grouped = {};
    for (const category of Object.keys(exports.MCP_CATEGORIES)) {
        grouped[category] = exports.MCP_REGISTRY.filter(s => s.category === category);
    }
    return grouped;
}
exports.mcpRegistry = {
    ...
    categories: exports.MCP_CATEGORIES
};
exports.default = exports.mcpRegistry;
